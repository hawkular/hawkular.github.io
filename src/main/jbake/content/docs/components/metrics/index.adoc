...
data points does little to improve the visualization, slows down the rendering, and makes the UI less response which in
turn makes the user experience worse overall. Downsampling is a way to return a predictable or fixed number of data
points which facilitates better graphs and a better overall user experience.

Hawkular Metrics provides several `/stats` endpoints that use downsampling.
...
We use for buckets to end up with:
...
`bucketDuration` is a duration in milliseconds that specifies how wide or large each bucket is. For the preceding
example, `bucketDuration=900000` specifies that each bucket is 15 minutes wide, which again yields four buckets. A
larger duration results in fewer buckets with more data points per bucket. A smaller duration results in more buckets
with less data points per bucket.
...
The `empty` property is true indicating that there were no data points in the bucket. The `NaN` values correspond to
Java's link:https://docs.oracle.com/javase/8/docs/api/java/lang/Double.html#NaN[Double.NaN].
...
----
curl -X GET http://server/hawkular/metrics/gauges/request_size/stats?start=1235&end=6789&bucketDuration=60000 \
-H "Content-Type: application/json" -H "Hawkular-Tenant: com.acme"
----
...
----
curl -X GET http://server/hawkular/metrics/counters/total_requests/stats?start=1235&end=6789&bucketDuration=60000 \
-H "Content-Type: application/json" -H "Hawkular-Tenant: com.acme"
----
...
----
curl -X GET http://server/hawkular/metrics/counters/total_requests/rate/stats?start=1235&end=6789&bucketDuration=60000 \
-H "Content-Type: application/json" -H "Hawkular-Tenant: com.acme"
----
...
time within the bucket's stat and end times.
...
`downtimeCount` is the number of times within the duration of the bucket that the metric was reported down.
...
----
curl -X GET http://server/hawkular/metrics/availability/server1/stats?start=1235&end=6789&bucketDuration=60000 \
-H "Content-Type: application/json" -H "Hawkular-Tenant: com.acme"
----
