...
Plugins are responsible to execute actions when an alert happens.
...
* Process an incoming action message wrapped as a `org.hawkular.alerts.actions.api.PluginMessage`
...
The class `org.hawkular.alerts.api.model.action.Action` is generated for the engine and it has the alert detail as
part of its payload.
