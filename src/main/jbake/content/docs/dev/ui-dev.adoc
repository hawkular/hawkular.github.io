...
... _Hint:_ when you run `gulp watch-server` with `--production`, output Javascript will be minified
...
.. _Hint:_ Hawkular UI tests uses https://angular.github.io/protractor/#/[protractor] higher than 2.x so you need to have https://nodejs.org/[node.js] higher than 4.x. You can follow this https://davidwalsh.name/upgrade-nodejs[guide] on how to upgrade your node.js version.
...
** e2e - directory with integration tests written in https://angular.github.io/protractor/#/[protractor], follow the file structure where `fileName.spec.ts` contains methods for tests and `fileName.e2e.spec.ts` contains tests.
