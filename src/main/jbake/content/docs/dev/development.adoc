...
TIP: If you want hot-deploy during development, you need to enable the deployment scanner at `standalone/configuration/standalone.xml` 
by setting `<deployment-scanner scan-enabled="true" ...`, remove the respective component from above modules directory and can 
then copy your deployment into the normal deployment directory at `standalone/deployments/`.
