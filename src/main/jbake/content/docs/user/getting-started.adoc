...
. In the `standalone/configuration/standalone.xml` file of your WildFly instance, make the following additions
.. Add the Hawkular Monitor Agent extension in the <extensions> section:
[source,xml]
----
<extension module="org.hawkular.agent.monitor"/>
----
. Add the Hawkular Monitor Agent subsystem declaration in the <profile> section. Note that you must set your Hawkular credentials in the username and password attributes (in other words, replace SET_ME with their true values for your Hawkular system):
[source,xml]
----
...
. Add an outbound socket binding to your WildFly instance's <socket-binding-group> that points to your running Hawkular server instance:
[source,xml]
----
