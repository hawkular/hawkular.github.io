...
The above instructions tell you how to secure your Hawkular Server and how the Hawkular WildFly Monitor agent can talk to the Hawkular Server over that secured communications channel. But this does nothing to secure the communications between the agent and the remote WildFly Servers the agent is monitoring. To secure this communications channel, the following must be performed.
...
The above only configured the managed WildFly Server itself. You then have to tell the Hawkular WildFly Monitor agent to talk to that managed server over the secure interface. You do this by first creating a security-realm in the agent's WildFly Server container with the managed WildFly Server's certificate in the given keystore (remember this is now editing the _agent's_ WildFly Server standalone.xml):
...
----
<managed-servers>
  <remote-dmr useSSL="true" securityRealm="ManagedWildFlyRealm" ... />
----
