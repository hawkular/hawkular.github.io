...
In this first demo we are going to develop and trace simple Spring Boot app.

== Create a web application
First let's write a simple web app, no let's generate it! All we have to do is just to select a web dependency.
...
key OpenTracing benefits like: vendor-neutrality, O(1) change of tracing system. In the next blog post we will look at
