...
lines randomly cause an exception which will result in the span (associated with the REST endpoint invocation)
being tagged as an error with associated log events identifying the error details.
...
Finally, the first two methods call the `/account` endpoint on _accountmgr_, while the third method invokes
a non-existent endpoint on _accountmgr_, which will lead to an error being reported in the trace and metric
data.
