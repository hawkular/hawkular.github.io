...
|startResolvedTime|No|Filter out alerts resolved before this time, millisecond since epoch.|integer|int64|-
|endResolvedTime|No|Filter out alerts resolved after this time, millisecond since epoch.|integer|int64|-
|startAckTime|No|Filter out alerts acknowledged before this time, millisecond since epoch.|integer|int64|-
|endAckTime|No|Filter out alerts acknowledged after this time, millisecond since epoch.|integer|int64|-
...
. link:#++POST__events++[Create a new Event. Persist the new event and send it to the engine for processing/condition evaluation.]
. link:#++POST__events_data++[Send events to the engine for processing/condition evaluation. Only events generated by the engine are persisted.Input events are treated as external data and those are not persisted into the system.]
...
NOTE: *Create a new Event. Persist the new event and send it to the engine for processing/condition evaluation.* +
...
NOTE: *Send events to the engine for processing/condition evaluation. Only events generated by the engine are persisted.Input events are treated as external data and those are not persisted into the system.* +
...
|Yes|Data to be processed by alerting.|array of <<Event>>
...
|eventType|No|-|string|-|-
|id|No|-|string|-|-
|lifecycle|No|-|array of <<LifeCycle>>|-|-
